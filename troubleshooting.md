...
- Ensure `Config.target.enabled = true`
- Verify `Config.target.system` matches your installed target resource
- Make sure the target resource starts **before** `ls_lootables`
...
## Can I use this with multiple frameworks?

Yes. `kq_link` auto-detects your framework. No framework configuration is needed in Lootables.
